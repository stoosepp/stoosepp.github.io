 ...
 */
    trial(display_element, trial) {
      ...
      function submit() {
        ...
        const this_level = gridArray[current_grid_index].level;
        //console.log(`This Level: ${this_level} - Last Level: ${last_level}`);
        var isPractice = true;
        if (last_level == this_level) {
          ...
          // console.log(`On the highest level of complexity. Is practice:${isPractice}`);
          var correctTileCount = 0;
          filledTiles.forEach((highlightedButton) => {
            if (correctTileIDs.includes(Number(highlightedButton.id)) == true) {
              correctTileCount++;
            }
          });
        ...
        if (isPractice == false) {
          ...
          var rt =
            performance.now() -
            (startTime + trial.stimulous_duration * 1000 + 1000);
          //console.log(`RT is ${performance.now()/1000} - ${startTime} - ${trial.stimulous_duration*1000} =  ${rt/1000} seconds`);
          gridData.push({
            grid_level: gridArray[current_grid_index].level,
            grid_trial: gridArray[current_grid_index].trial,
            grid_rt: rt,
            correct: correct,
          });
